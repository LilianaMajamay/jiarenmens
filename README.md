...
2. **持仓/调仓需关注选手**：不加 `--follow` 时大多返回"关注关系不存在"（9035）。
   ⚠️ **`--follow` 有自选组合数量上限**：每关注一个选手，他就会被加入你的
   "自选组合"榜，该榜有**数量上限**，关注满后无法再关注新组合，后续选手的
   持仓/调仓都会失败。因此**不要一次性大批量 `--follow` 爬取**，建议：
   - 分批爬取：`python main.py --follow --limit 50` 爬一批；
   - 处理完后**一键取消关注**已处理选手，再跑下一批：
     `python main.py --unfollow`（会清空全部已关注组合，释放名额）；
   - 结合断点续传（检查点），分多次执行不会重复爬已完成的选手；
   - 对未授权公开（30001）的选手，关注了也看不到数据，跳过即可
